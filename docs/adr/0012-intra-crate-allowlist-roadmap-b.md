...
| **Perm** | `application/container.rs` + `infrastructure::observability` remain | 0 | 0 | — | **Never removed** | — | Permanent |
    ...
    **How the 8 removable entries map to slices** (Sites column = exclusive absorption measured on `8dc58c6e`, §1.2.1):
    ...
    | `application/asset_download.rs` | **Follow-up to 3.B-1b** | `asset_download::Downloader::new` → `DownloaderFactory::build` via injected `Arc<dyn DownloaderFactory>` | 1 |
    ...
    | `application/crawler/engine.rs` | **3.F** (then `SystemRamProbe` default hoist if `cli` becomes sole ctor) | `engine.rs: DomainSessionPool` + `engine.rs: SystemRamProbe` → `domain::session_port` + `domain::ram_probe_port` | 4 (3 + 1 default) |
