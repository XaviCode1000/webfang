...
| offline | N/T | BLOCKED → **PASS** (`--offline` warm = 0, cold = exit 78 Spanish, no silent fallback) | N/T | BLOCKED → **N/T** | N/A |
...
| Real large page (wikipedia) | PARTIAL | N/T | chunks end mid-sentence (**MD-4**) | — |
...
| Invalid model id (`--ai-model` / `AI_MODEL_ID`) | PASS | N/A | pre-fetch fail-fast | exit 64, Spanish, lists valid ids |
...
| `--output-vectors` alongside `--clean-ai` | **FAIL** | N/A | vectors ≠ corpus chunks; duplicate fetch | — (**MD-1**) |
...
Opened by this run: **MD-1 → #1310**, **MD-2 → #1311**, **MD-3 → #1312**, **MD-4 → #1313**.
