...
use rust_scraper_core::domain::value_objects::CorrelationId;
use rust_scraper_core::domain::DomainError;
use rust_scraper_core::error::ScraperError;
...
fn tracestate_format() {
    ...
    assert!(
        ts.starts_with("rust_scraper=v1:"),
        "tracestate should use vendor format"
    );
    // Total length: "rust_scraper=v1:" (16) + 32 hex = 48
    assert_eq!(ts.len(), 48);
}
