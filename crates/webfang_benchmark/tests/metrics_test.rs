...
fn success_rate_is_succeeded_over_total() {
    ...
    let metrics = compute(&records, webfang_core::domain::JsStrategy::Static, 0).expect("computes");
    assert!(
        (metrics.success_rate - 0.7).abs() < f64::EPSILON,
        "7/10 must be 70%"
    );
}
...
fn missing_summary_is_error() {
    ...
    let err =
        compute(&records, webfang_core::domain::JsStrategy::Static, 0).expect_err("no summary");
    assert!(matches!(
        err,
        webfang_benchmark::BenchmarkError::MissingSummary { .. }
    ));
}
