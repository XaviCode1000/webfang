    ...
    async fn verify_waf_integrity(
        ...
        if let Some(ref hdrs) = params.headers {
            for (key, value) in hdrs {
                header_map.insert(key.to_lowercase(), value.clone());
            }
        }
...
fn detect_waf_provider(html: &str) -> Option<&'static str> {
    let verdict = waf_inspector().inspect(html, &InspectionContext::default());
    if verdict.is_blocked {
...
mod tests {
    ...
    /// Mirrors the composition root: `detect_waf_provider` reads the
    /// process-wide inspector installed by `build_container` (#996). Unit
    /// tests bypass the container, so they install the real infrastructure
    /// impl explicitly. Idempotent (`set_waf_inspector` keeps the first
    /// value), so test execution order is irrelevant.
    fn install_real_inspector() {
        static INSTALL: Once = Once::new();
        INSTALL.call_once(|| {
            webfang_core::domain::waf::set_waf_inspector(std::sync::Arc::new(
                webfang_core::infrastructure::http::waf_engine::WafInspector,
            ));
        });
    }
