...
use rust_scraper_core::application::container::Container;
use rust_scraper_core::domain::CrawlerConfig;
use rust_scraper_core::infrastructure::config::ScraperConfig;
use rust_scraper_mcp::mcp_server::state::{CategoryLimits, CategorySemaphores, McpState};
use rust_scraper_mcp::mcp_server::McpHandler;
...
fn scrape_url_params_deserialize_valid() {
    ...
    let params: rust_scraper_mcp::mcp_server::params::ScrapeUrlParams =
...
fn scrape_url_params_rejects_missing_url() {
    let json = r#"{}"#;
    let result =
        serde_json::from_str::<rust_scraper_mcp::mcp_server::params::ScrapeUrlParams>(json);
...
fn crawl_site_params_deserialize() {
    ...
    let params: rust_scraper_mcp::mcp_server::params::CrawlSiteParams =
        serde_json::from_str(json).unwrap();
...
fn clean_html_params_deserialize() {
    ...
    let params: rust_scraper_mcp::mcp_server::params::CleanHtmlParams =
        serde_json::from_str(json).unwrap();
...
fn crawl_with_sitemap_params_optional_fields() {
    let json = r#"{"url": "https://example.com"}"#;
    let params: rust_scraper_mcp::mcp_server::params::CrawlWithSitemapParams =
        serde_json::from_str(json).unwrap();
...
fn scrape_batch_params_deserialize() {
    ...
    let params: rust_scraper_mcp::mcp_server::params::ScrapeBatchParams =
        serde_json::from_str(json).unwrap();
...
fn detect_spa_params_deserialize() {
    let json = r#"{"url": "https://example.com"}"#;
    let params: rust_scraper_mcp::mcp_server::params::DetectSpaParams =
        serde_json::from_str(json).unwrap();
    assert_eq!(params.url, "https://example.com");
}
...
fn discover_urls_params_deserialize() {
    let json = r#"{"url": "https://example.com"}"#;
    let params: rust_scraper_mcp::mcp_server::params::DiscoverUrlsParams =
        serde_json::from_str(json).unwrap();
    assert_eq!(params.url, "https://example.com");
}
...
fn export_file_params_deserialize() {
    ...
    let params: rust_scraper_mcp::mcp_server::params::ExportFileParams =
        serde_json::from_str(json).unwrap();
...
fn detect_vault_params_optional_path() {
    let json = r#"{}"#;
    let params: rust_scraper_mcp::mcp_server::params::DetectVaultParams =
        serde_json::from_str(json).unwrap();
