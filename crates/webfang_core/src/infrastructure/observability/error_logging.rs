...
/// Log a structured error carrying its operational [`ErrorClass`](crate::domain::error::ErrorClass).
///
/// Same contract as [`log_scrape_error`], plus a `class` tracing field so
/// the runner policy per classification is observable without parsing the
/// message (#865). The class is recorded via `Debug` (`TransientBackoff`)
/// and its snake_case spelling is available through `Display` on the error
/// value itself when callers pass it as `error`.
///
/// # Arguments
/// * `error` - The error to log (any `Display` type; the classification
///   itself when no richer payload exists).
/// * `class` - Operational classification decided where the error was born.
/// * `url` - The URL being processed when the error occurred.
/// * `stage` - The stage/phase where the error occurred.
